...
type Action = {
  ...
  featured?: boolean
  expandType?: 'headlines' | 'trustbar' | 'customer' | 'whyus' | 'casestudy' | 'buttons'
}
...
type WhyUsCopy = {
  intro: string
  differentiators: Array<{ headline: string; subtext: string; note?: string }>
}
...
type ButtonAlternatives = {
  intro: string
  primary: Array<{ text: string; rationale: string }>
  secondary: Array<{ text: string; rationale: string }>
}

type AuditOverviewViewProps = {
  isPaid: boolean
...
export function AuditOverviewView({
  ...
  return (
    <div role="tabpanel" id="tabpanel-overview" aria-labelledby="tab-overview">
            ...
            {actions.map((action) => (
              ...
              >
                <div
                  ...
                  onClick={() => {
                    if (action.priority > freeActionLimit && !isPaid) {
                      onUnlock()
                    } else {
                      onActionExpand(action.priority)
                    }
                  }}
                >
                    ...
                    <div className="flex flex-wrap items-center gap-3 mb-2">
                      ...
                      {action.expandType && (
                        <span className="text-xs text-[var(--accent)] font-semibold">
                          {action.priority > freeActionLimit && !isPaid ? (
                            '🔒 Unlock to see copy'
                          ) : (
      ...
      <ViewNavBar prevView={null} nextView={{ id: 'message', label: 'Your message', description: 'How clearly you communicate what you do and why it matters' }} onNavigate={onViewChange} />
    </div>
  )
}
