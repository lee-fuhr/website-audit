...
import { auditData } from './sampleData'
...
// Sample page - always shows full content (no locking)
export default function SamplePage() {
  ...
  // Sample mode: always show full content
  const isPaid = true
  ...
  const handleScoreCardClick = (key: string) => {
    setActiveModal(key as keyof typeof auditData.scores)
    window.plausible?.('Score Card Clicked', { props: { category: auditData.scores[key as keyof typeof auditData.scores].label } })
  }
  ...
  return (
    <main className="min-h-screen bg-[var(--background)]">
      ...
      {/* Sticky TOC - offset for sample banner */}
      <nav className="hidden lg:block print:!hidden fixed top-[44px] left-0 w-64 h-[calc(100vh-44px)] bg-[var(--accent)] text-white p-8 overflow-y-auto z-40 flex flex-col">
        <div className="mb-10">
          <p className="text-xs uppercase tracking-wider opacity-60 mb-1">Sample audit for</p>
          <button
        ...
        <div className="mt-auto pt-8 border-t border-white/20 print:hidden">
          <Link
            href="/"
            className="w-full py-3 px-4 text-sm bg-white text-[var(--accent)] font-bold hover:bg-white/90 transition-all flex items-center justify-center gap-2"
          >
            Analyze your site →
          </Link>
        </div>
      ...
      {/* Main content - offset for TOC and sample banner */}
      <div className="lg:ml-64 print:!ml-0 pt-[44px]">
        {/* Header */}
        ...
        {/* Sample CTA section before footer */}
        <section className="section bg-[var(--muted)] print:hidden">
          <div className="container">
            <div className="max-w-2xl mx-auto text-center">
              ...
              <Link
                href="/"
                className="inline-block bg-[var(--accent)] text-white px-10 py-5 text-xl font-bold hover:opacity-90 transition-opacity"
              >
                Analyze your site →
              </Link>
            </div>
        ...
        <Footer context={`Sample audit for ${auditData.company}`} />
      </div>
    </main>
  )
}
